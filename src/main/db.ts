...
export function ingest(items: IngestItem[]): number {
  const stmt = db.prepare(`INSERT INTO items
    (source, ext_id, kind, deep_link, app_link, title, sender, snippet, body, source_ts, state, created_at, last_touched_at)
    VALUES (@source,@ext_id,@kind,@deep_link,@app_link,@title,@sender,@snippet,@body,@source_ts,'new',@ts,@ts)
